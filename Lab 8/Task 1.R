...
setwd("/Users/resh/Downloads/DS/DS_TEB2043/Lab 8")

titanic_lab8a <- read.csv("titanic.csv")

# 1. Changed 'Fare' to 'fare'
titanic_sortbyfare = arrange(titanic_lab8a, desc(fare))

write.csv(titanic_sortbyfare, "titanic_sortbyfare.csv")

titanic_lab8b <- read.csv("titanic_sortbyfare.csv")

# 2. Added dplyr:: and made all column names lowercase
titanic_clean <- titanic_lab8b %>%
  dplyr::select(survived, pclass, sex, age, fare, embarked)

write.csv(titanic_clean, "titanic_clean.csv")

#general insights (Updated to lowercase $survived)
...
#age groups (Updated to lowercase $age)
...
#survival rate by age (Updated to lowercase $survived)
...
#embarked from Cherbourg (Updated to lowercase $embarked, $survived, $pclass)
...
#passengers who paid 0 fare (Updated to lowercase $fare, $pclass)
...
#youngest passenger (Updated to lowercase $age, $survived)
...
#female First Class Survival (Updated to lowercase $sex, $pclass, $survived)
...
#male First Class Survival (Updated to lowercase $sex, $pclass, $survived)
